...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        masking = [True if i < len(sentence) else False for i in range(max_length)]
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        features = tf.nn.embedding_lookup(embedding_tensor, self.input_placeholder)
    ...
    def add_prediction_op_gru(self, x, dropout_rate):
        ...
        output_layer = tf.layers.dense(units=self.config.n_classes, name='FFNN',
                                       kernel_initializer=tf.contrib.layers.xavier_initializer(),
                                       bias_initializer=tf.constant_initializer(0.))
    ...
    def add_summary_op(self, pred, loss):
        ...
        clip_probs = tf.clip_by_value(self.probs,0.01)
